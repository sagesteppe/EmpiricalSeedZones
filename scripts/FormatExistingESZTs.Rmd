...
wrapper(
  x = st_read(fnames[[grep('Artemisia_tridentata-WUS', fnames)]]) |> select(SeedZone = Seedzone),
...
wrapper(cllu2, SeedZone = 'Axis', sci_name = 'Cleomella_lutea', 
        nrcs_code = 'CLLU2',
        estz_type = 'lg', caption = 'Data from Massatti et al. 2020. doi: 10.1111/rec.13142')
...
NOTE ACHY Is a weird one... the gridcode is a non consecutive range of numbers not starting from 0 or 1, and the SZNames are a consecutive version of these numbers starting from 1. 
We will overwrite the SZName to make them confluent with our 'new' values which are based on aridity.
To do this we just omit the original zone column. 

ACHY IS a dupe of THURBERS??? CHECK FIX 

```{r eriocoma hymenoides, eval = F}
...
```{r Sphaeralcea parvifolia surfaces, eval = F}
