...
template <- rast(x = w_ext, crs = "epsg:5070", res = 1/360) # 10 arc second
names(template) <- names(esz)

fp <- '../data/geodata/rasters/'

for(i in 1:length(esz)){ ## need to write each tif seperately!!!! 
  rasterize(vect(esz[[i]]), template, field = 'Zone', filename = paste0(fp, names(esz[i]), '.tif'))
}
