...
binom_plot <- filenames %>% 
  ...
  labs(
    x = NULL,
    y = 'Count', 
    title = 'How do filenames represent the species in them', 
...
filenames %>% 
  ...
  ggplot(., aes(y = Value, x = '', fill = forcats::fct_rev(Response))) + 
  geom_bar(position = "stack", stat = "identity") + 
  labs(y = 'Count', x = NULL, title = 'Is the Geographic Extent of the Seed Zones specified?', 
       fill = 'Specified') +
  theme_minimal() +
  theme(
...
filenames %>% 
  ...
  ggplot(., aes(y = Value, x = '', fill = forcats::fct_rev(Response))) + 
  geom_bar(position = "stack", stat = "identity") + 
  labs(y = 'Count', x = NULL, title = 'Do files mention their subject matter?', 
