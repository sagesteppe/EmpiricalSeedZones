...
binom_plot <- filenames %>% 
  ...
  scale_fill_manual(values = c('#686963', '#EF476F', '#758BFD',  '#06D6A0'))
...
extent_plot <- filenames %>% 
  ...
  geom_text(aes(label = Response), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values = c('#686963',  '#06D6A0')) 
...
file_plot <- filenames %>% 
  ...
  labs(y = 'Count', x = NULL, title = 'Do files mention\nthey contain STZs\n if so how?', 
    ...
    scale_fill_manual(values = c('#686963', '#758BFD', '#FF8600',  '#06D6A0'))+
  theme_minimal() +
  theme(
    legend.position="none", 
    plot.title = element_text(hjust = 0.5),
...
fnames <- binom_plot  + file_plot + extent_plot + 
  plot_annotation(title = 'Are there consistent file name components?', 
                  theme = theme(plot.title = element_text(size = 18, hjust = 0.5))) 

fnames
ggsave('../poster/filenames.png', fnames)
...
ggplot(data = maps, aes(fill = Response, y = Value, x = Trait)) + 
  ...
  scale_fill_manual(values = c('#758BFD','#EF476F')) + 
  theme_classic() + 
...
ggplot() + 
  ...
  geom_sf(data = nrcs_poly,  linetype = 2, fill = 'orange4', alpha = 0.2) +  
  geom_sf(data = stz_poly, linetype = 2, fill = 'blue2', alpha = 0.2) +  
  geom_sf(data = region_poly,  linetype = 2, fill = 'red', alpha = 0.2) + 
  geom_sf(data = ext_poly, linetype = 2, fill = 'maroon', alpha = 0.2 ) + 
...
ggsave('../poster/ExampleFileName.png', height = 8, width = 8, units = 'in', dpi = 300)
...
areas_plot <- vals %>% 
  ...
  labs(y = 'Count', x = NULL, title = 'Is an "Area" field Present-\nand labelled?', 
       fill = 'Multipolygon') + 
    scale_fill_manual(values = c('#686963', '#758BFD', '#06D6A0'))+
  theme_minimal() +
    scale_x_discrete(expand = c(0,0)) +
  theme(
    legend.position="none", 
...
fields <- polygons_plot + zones_plot + areas_plot + 
  plot_annotation(title = 'Are field names consistent across files?', 
                  theme = theme(plot.title = element_text(size = 18, hjust = 0.5))) 
...
```{r Example of fields data, eval = F}

fields <- data.frame(Label = 'ID | SeedZone | SZName | AreaAcres', x = 0, y = 0) |>
  st_as_sf(coords = c(x = 'x', y = 'y'))

ID_Poly <- st_polygon(
  list(
    rbind(
      c(-0.05075,-0.0025), c(-0.05075,0.0095),  c(-0.08,0.0095),  c(-0.08,-0.0025), 
      c(-0.05075,-0.0025))
    )
  ) |>
  st_buffer(dist = 0.0005,
...
SeedZone_poly <- st_polygon(
  list(
    rbind(
      c(-0.050, -0.0025), c(-0.050, 0.0105), c(-0.065,0.0105), c(-0.065,0.0195), 
      c(-0.01175, 0.0195), c(-0.01175, -0.0025), c(-0.050,-0.0025))
    )
  ) |> 
  st_buffer(dist = 0.0005,
  joinStyle = 'ROUND', 
...
SZName_poly <- st_polygon(
  list(
    rbind(
      c(-0.01075, -0.0025), c(-0.01075, 0.0195), 
      c(0.0425, 0.0195), c(0.0425, 0.0095), c(0.022, 0.0095), 
      c(0.022, -0.0025), c(-0.01075, -0.0025)) 
    )
  ) |> 
  st_buffer(dist = 0.0005,
  joinStyle = 'ROUND', 
...
AreaAcres_poly <- st_polygon(
  list(
    rbind(
      c(0.023, -0.0025), c(0.023, 0.0085), c(0.070, 0.0085), c(0.070, -0.0025),
      c(0.023, -0.0025)) 
    )
  ) |> 
  st_buffer(dist = 0.0005,
...
ggplot() + 
  geom_sf_text(data = fields, aes(label = Label), fontface = 'bold', size = 8) +  
   labs(x = NULL, y = NULL, title = 'Contents and Naming Conventions for Fields') +  
  theme_void() +
  theme(plot.title = element_text(hjust = 0.5)) + 
  
  geom_sf(data = ID_Poly,  linetype = 2, fill = 'orange4', alpha = 0.2) +
  geom_sf(data = SeedZone_poly,  linetype = 2, fill = 'red', alpha = 0.2) +
  geom_sf(data = SZName_poly,  linetype = 2, fill = 'green', alpha = 0.2) +
  geom_sf(data = AreaAcres_poly,  linetype = 2, fill = 'purple', alpha = 0.2) +
  
  coord_sf(ylim = c(-0.02, 0.02), xlim = c(-0.08, 0.08)) 
  
ggsave('../poster/ExampleFields.png', height = 8, width = 8, units = 'in', dpi = 300)
```
