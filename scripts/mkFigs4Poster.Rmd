...
zones_plot <- vals %>% 
  ...
  geom_bar(position = "stack", stat = "identity", width = 1) + 
  labs(y = 'Count', x = NULL, title = 'Does the field with Seed Zone\ninformation have "Zone" in the name?', 
       fill = 'Zone') + 
    scale_fill_manual(values = c('#C0C5C1',  '#496F5D'))+
  theme_minimal() +
  theme(
...
polygons_plot <- vals %>% 
  ...
  ggplot(., aes(y = Value, x = '', fill = Response)) + 
  geom_bar(position = "stack", stat = "identity", width = 1) + 
  labs(y = 'Count', x = NULL, title = 'Are the geometries multipolygons?', 
       fill = 'Multipolygon') + 
    scale_fill_manual(values = c('#C0C5C1',  '#496F5D'))+
  theme_minimal() +
  theme(
...
areas_plot <- vals %>% 
  ...
  labs(y = 'Count', x = NULL, title = 'Are the geometries multipolygons?', 
       fill = 'Multipolygon') + 
    scale_fill_manual(values = c('#C0C5C1',  'green', '#496F5D'))+
  theme_minimal() +
  theme(
    legend.position="bottom", 
    plot.title = element_text(hjust = 0.5),
...
fnames <- polygons_plot  + zones_plot + areas_plot + 
  plot_annotation(title = 'How are fields named in files?', 
                  theme = theme(plot.title = element_text(size = 18, hjust = 0.5))) 
